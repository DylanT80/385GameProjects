...
public class EggAction : MonoBehaviour
{
    ...
    {
        // Destroy only if not colliding on waypoint OR
        // it is a waypoint and it is not disabled
        if (other.tag != "Waypoint" || other.tag == "Waypoint" && other.gameObject.GetComponent<SpriteRenderer>().enabled)
            DestoryProjectile();
    }
